import cv, { type ContourWrapper, type WrappedMat } from './opencvAdapter';
import { Skia, type SkPath } from '@shopify/react-native-skia';
import {
  BASEBOARD_SEMANTIC_NAME,
  classifyBgrPixelToSemantic,
  getCabinetQuantKeys,
  getSemanticColorByName,
  getWallQuantKeys,
  getBaseboardStripQuantKeys,
  isStrictBaseboardPixel,
...
type RowRun = { minX: number; maxX: number; y: number };

function bboxToPolygon(bbox: SegmentRegion['bbox']): NormPoint[] {
  return [
    { x: bbox.x, y: bbox.y },
...
export function buildRegionOutlinePolygons(reg: SegmentRegion): NormPoint[][] {
...
function quantChannelSlot(value: number): number {
  const q = Math.min(
    255,
    Math.round(value / maskCfg().quantStep) * maskCfg().quantStep,
  );
  if (q >= 192) {
    return q === 255 ? 4 : 3;
  }
...
function quantKeyIndex(b: number, g: number, r: number): number {
  return (
    quantChannelSlot(b) * 25 + quantChannelSlot(g) * 5 + quantChannelSlot(r)
  );
}

function getStripQuantIndices(): Set<number> {
  ...
  cachedStripQuantIndices = new Set(
    [...getBaseboardStripQuantKeys()].map(key => {
...
let cachedStripQuantIndices: Set<number> | null = null;

let channelSlotLutRevision = -1;
let cachedChannelSlotLut: Uint8Array | null = null;

function buildQuantChannelSlotLut(quantStep: number): Uint8Array {
  const lut = new Uint8Array(256);
  for (let value = 0; value < 256; value++) {
    const q = Math.min(
      255,
      Math.round(value / quantStep) * quantStep,
    );
    if (q >= 192) {
      lut[value] = q === 255 ? 4 : 3;
    } else if (q >= 128) {
...
function getQuantChannelSlotLut(): Uint8Array {
  const revision = getMaskRuntimeRevision();
...
function quantSlotToChannel(slot: number): number {
  if (slot >= 4) {
    return 255;
  }
  if (slot >= 3) {
...
function quantIndexToBgr(idx: number): [number, number, number] {
...
let cachedSemanticLut: Uint8Array | null = null;

let cachedStripQuantLut: Uint8Array | null = null;
let stripQuantLutRevision = -1;

function getStripQuantLut(): Uint8Array {
  const revision = getMaskRuntimeRevision();
...
function bboxFromBinary(
  binary: Uint8Array,
  cols: number,
  rows: number,
): SegmentRegion['bbox'] | null {
  let minX = cols;
  let minY = rows;
  let maxX = -1;
  let maxY = -1;
...
export function appendMaskBinaryToPathBuilder(
  ...
  minRunPx = maskCfg().baseboardMinRunPx,
): void {
  for (let y = 0; y < rows; y++) {
    let runStart = -1;
    const normY0 = y / rows;
...
function appendRunRectToBuilder(
  runStart: number,
  runEnd: number,
  y: number,
  cols: number,
...
export function buildAllRegionMaskPaths(
  ...
  const { labels, baseboardBinary, cols, rows } = maskData;
  const builders = new Map<number, PathBuilderLike>();
  const semanticColors = getMaskSegmentRuntimeConfig().mask.semanticColors;
  const regionIdBySemantic = new Int32Array(semanticColors.length);
  regionIdBySemantic.fill(-1);
  let baseboardRegionId: number | null = null;
  ...
  for (const reg of regions) {
    builders.set(reg.id, Skia.Path.Make() as unknown as PathBuilderLike);
    if (reg.thinStrip) {
      baseboardRegionId = reg.id;
      continue;
    }
      ...
      if (baseboardRegionId != null) {
        ...
        if (!bbOn && baseboardRunStart >= 0) {
          appendRunRectToBuilder(
            ...
            rows,
            rect,
            builders.get(baseboardRegionId)!,
            minRunPx,
          );
          baseboardRunStart = -1;
        }
  ...
  const paths = new Map<number, SkPath>();
  for (const [regionId, builder] of builders) {
    paths.set(regionId, builder as unknown as SkPath);
  }
  return paths;
}
...
function bboxFromPolygons(polygons: NormPoint[][]): SegmentRegion['bbox'] | null {
...
function extractBaseboardRowPolygons(
  ...
  polygons: NormPoint[][];
  hitPolygons: NormPoint[][];
  totalArea: number;
  bbox: SegmentRegion['bbox'] | null;
} {
  let totalArea = 0;
  ...
  const bridgedRuns = collectRowRuns(
    bridged,
    cols,
    rows,
    maskCfg().baseboardMinRunPx,
  );
...
function cloneBinary(binary: Uint8Array): Uint8Array {
  return new Uint8Array(binary);
}

function subtractBinary(target: Uint8Array, mask: Uint8Array): void {
...
function minPalettePixels(cols: number, rows: number): number {
  return Math.max(300, Math.floor((cols * rows) / 2000));
}

function minPixelsForSemantic(
  name: string,
  cols: number,
  rows: number,
): number {
  ...
  return Math.max(
    base,
    Math.floor(cols * rows * maskCfg().secondaryMinPixelRatio),
  );
}
...
function maskQuantKey(b: number, g: number, r: number): string {
  return `${quantizeChannel(b)},${quantizeChannel(g)},${quantizeChannel(r)}`;
}

function dilateBinaryBox(
  source: Uint8Array,
  cols: number,
  rows: number,
  radiusX: number,
...
function buildWallCabinetJunctionMask(
  buffer: Uint8Array,
  cols: number,
  rows: number,
): Uint8Array {
  ...
  const wallNear = dilateBinaryBox(
    wall,
    cols,
    rows,
    maskCfg().junctionHRadiusPx,
    maskCfg().junctionVRadiusPx,
  );
  const cabinetNear = dilateBinaryBox(
    cabinet,
    cols,
    rows,
    maskCfg().junctionHRadiusPx,
...
function computeStrictBaseboardBand(
...
function isJunctionNearStrictBaseboard(
  ...
  const y = (idx - x) / cols;
  if (
    y < band.minY - maskCfg().baseboardJunctionRowMarginPx ||
  ...
  const halfW = maskCfg().kickBridgeHalfWPx;
  for (
    let dy = -maskCfg().baseboardJunctionVReachPx;
    dy <= maskCfg().baseboardJunctionVReachPx;
    dy++
  ) {
    const ny = y + dy;
    if (ny < 0 || ny >= rows) {
...
function buildBaseboardBinary(
  buffer: Uint8Array,
  cols: number,
  rows: number,
  junctionMask?: Uint8Array,
): Uint8Array {
  const binary = new Uint8Array(cols * rows);
  const junction =
    junctionMask ?? buildWallCabinetJunctionMask(buffer, cols, rows);
  ...
  for (let y = 0; y < rows; y++) {
    for (let x = 0; x < cols; x++) {
      ...
      const key = maskQuantKey(b, g, r);
      if (
        getBaseboardStripQuantKeys().has(key) &&
        junction[idx] &&
...
export function buildBaseboardBinaryFromMask(
  buffer: Uint8Array,
  cols: number,
  rows: number,
): Uint8Array {
...
function buildMaskPolygonsFromBinary(
  binary: Uint8Array,
  cols: number,
  rows: number,
): NormPoint[][] {
  return rowRunsToPolygons(
    collectRowRuns(binary, cols, rows, maskCfg().baseboardMinRunPx),
    cols,
    rows,
  );
}

export function isBaseboardMaskPixel(
  buffer: Uint8Array,
  cols: number,
  rows: number,
  x: number,
  y: number,
  baseboardBinary?: Uint8Array | null,
): boolean {
...
export function getMaskQuantKey(b: number, g: number, r: number): string {
...
function isIgnoredColor(b: number, g: number, r: number): boolean {
  const threshold = maskCfg().blackThreshold;
  return b < threshold && g < threshold && r < threshold;
}

function countBinaryPixels(binary: Uint8Array): number {
  let count = 0;
  for (let i = 0; i < binary.length; i++) {
    if (binary[i]) {
      count += 1;
...
let cachedNameToIndex: Map<string, number> | null = null;

function getSemanticNameToIndex(): Map<string, number> {
  const revision = getMaskRuntimeRevision();
  ...
  cachedNameToIndex = new Map(
    colors.map((entry, index) => [entry.name, index]),
  );
  nameToIndexRevision = revision;
  return cachedNameToIndex;
}

function createSemanticLut(): Uint8Array {
  const revision = getMaskRuntimeRevision();
...
type SemanticLayout = {
  ...
  bboxes: Map<string, SegmentRegion['bbox']>;
  stripIndices: number[];
  strictBaseboard: Uint8Array;
...
function buildSemanticLayout(
  ...
  const bboxes = new Map<string, SegmentRegion['bbox']>();
  const semanticLut = createSemanticLut();
  const nameToIndex = getSemanticNameToIndex();
  const indexToName = getMaskSegmentRuntimeConfig().mask.semanticColors.map(
    entry => entry.name,
  );
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      const lutIdx =
        channelSlotLut[b] * 25 + channelSlotLut[g] * 5 + channelSlotLut[r];
...
function buildJunctionAtStripPixels(
  labels: Uint8Array,
  stripIndices: number[],
  cols: number,
  rows: number,
): { junction: Uint8Array; junctionIndices: number[] } {
  const junction = new Uint8Array(cols * rows);
...
function finalizeBaseboardBinary(
  strictBaseboard: Uint8Array,
  strictCount: number,
  junctionIndices: number[],
  cols: number,
  rows: number,
): { binary: Uint8Array; pixelCount: number } {
...
function buildPickMapAndWorkAreas(
  labels: Uint8Array,
  indexToName: string[],
  ...
  cols: number,
  rows: number,
): { pick: Uint8Array; workAreas: Map<string, number> } {
  const pixelCount = cols * rows;
  ...
  const baseboardCode =
    baseboardId === undefined ? 0 : baseboardId + 1;
  ...
  for (let i = 0; i < pixelCount; i++) {
    if (baseboardBinary[i]) {
      ...
      workAreas.set(
        baseboardName,
        (workAreas.get(baseboardName) ?? 0) + 1,
      );
      continue;
    }
...
function dilatePickBuffer1px(
  ...
  for (let y = 1; y < rows - 1; y++) {
    for (let x = 1; x < cols - 1; x++) {
      ...
      const n = [
        pick[(y - 1) * cols + (x - 1)], // top-left
        pick[(y - 1) * cols + x],       // top
        pick[(y - 1) * cols + (x + 1)], // top-right
        pick[y * cols + (x - 1)],       // left
        pick[y * cols + (x + 1)],       // right
        pick[(y + 1) * cols + (x - 1)], // bottom-left
        pick[(y + 1) * cols + x],       // bottom
...
function paletteFromCounts(
  counts: Map<string, number>,
  cols: number,
  rows: number,
): PaletteEntry[] {
  const orderedSemantics = getMaskSegmentRuntimeConfig().mask.semanticColors.map(
    entry => entry.name,
  );
  ...
  return orderedSemantics
    .map(name => {
      ...
      const ref = getSemanticColorByName(name) as MaskSemanticColor;
      return {
...
async function contourToPolygon(
  ...
  const epsilonPx = Math.max(
    1.5,
    Math.min(
      perimeter * approxEpsilon,
      maxEpsilonPx,
      Math.max(2, thinSide * 0.12),
    ),
  );
...
function extractPolygonsFromBinaryJs(
  binary: Uint8Array,
  cols: number,
  rows: number,
  minArea: number,
  ...
  const visited = new Uint8Array(cols * rows);
  const polygons: NormPoint[][] = [];
  let totalArea = 0;
  let bbox: SegmentRegion['bbox'] | null = null;
...
async function extractPolygonsFromBinary(
  binary: Uint8Array,
  cols: number,
  rows: number,
  minArea: number,
  ...
  try {
    ...
    for (const contour of contours) {
      try {
        const part = await contourToPolygon(
          contour,
          cols,
          rows,
          minArea,
...
export async function extractRegionsFromMaskBuffer(
  buffer: Uint8Array,
  cols: number,
  rows: number,
  _options: {
...
export function extractRegionsFromMaskBufferSync(
  buffer: Uint8Array,
  cols: number,
  rows: number,
  _options: {
  ...
  const { junctionIndices } = buildJunctionAtStripPixels(
    layout.labels,
    layout.stripIndices,
    cols,
    rows,
  );
  const { binary: baseboardBinary, pixelCount: baseboardPixels } =
    finalizeBaseboardBinary(
      layout.strictBaseboard,
      layout.strictBaseboardCount,
      junctionIndices,
      cols,
      rows,
    );
  ...
  const regionResults = paletteEntries.map(
    (entry): SegmentRegion | null => {
  ...
  const regions = regionResults.filter(
    (region): region is SegmentRegion => region != null,
  );
  ...
  const { pick: pickBufferRaw, workAreas } = buildPickMapAndWorkAreas(
    layout.labels,
    indexToName,
    nameToId,
    baseboardBinary,
